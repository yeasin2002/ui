...
## Testing `create` with Preset URLs (Local Registry)

When testing the `create` command with preset URLs locally, you need to point to your local dev server instead of production.

### Setup

```bash
# Terminal 1: Start the v4 docs site (serves local registry at http://localhost:4000)
pnpm v4:dev

# Terminal 2: Build the CLI
pnpm shadcn:build
```

### Testing Preset URLs Locally

Replace `https://ui.shadcn.com` with `http://localhost:4000` in preset URLs.

> **⚠️ Windows CMD Note:** The `&` character is a command separator in CMD. Use PowerShell or escape with `^`.

**PowerShell (recommended on Windows):**

```powershell
# Local testing with preset URL (note: template parameter is required in URL)
pnpm shadcn create my-app --preset 'http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist-sans&menuAccent=subtle&menuColor=default&radius=default&template=next' --template next -c "C:/Yeasin/experiment/test-preset"
```
...
```bash
# Production command (what users run):
bunx --bun shadcn@latest create my-app --preset "https://ui.shadcn.com/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist-sans&menuAccent=subtle&menuColor=default&radius=default&template=next" --template next

# Local testing equivalent:
pnpm shadcn create my-app --preset "http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist-sans&menuAccent=subtle&menuColor=default&radius=default&template=next" --template next -c ~/experiment/test-preset
```

### Test Cases for Preset URL Validation

#### ✅ Test: Geist Sans font (Issue #9066 fix)

```bash
# Clean test directory
rmdir /s /q C:\Yeasin\experiment\test-preset 2>nul
mkdir C:\Yeasin\experiment\test-preset

# Test with geist-sans font (correct value)
pnpm shadcn create test-geist --preset "http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist-sans&menuAccent=subtle&menuColor=default&radius=default" --template next -c "C:/Yeasin/experiment/test-preset"

# Expected: Success! Project initialization completed.
```

#### ❌ Test: Invalid font value (should fail with clear error)

```bash
# This should fail with validation error
pnpm shadcn create test-invalid --preset "http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=invalid-font&menuAccent=subtle&menuColor=default&radius=default" --template next -c "C:/Yeasin/experiment/test-preset"

# Expected: Error about invalid font value
```

#### ✅ Test: All available fonts

```bash
# Test each font value to ensure they all work:
# geist-sans, inter, noto-sans, nunito-sans, figtree, roboto, raleway, dm-sans, public-sans, outfit, jetbrains-mono

pnpm shadcn create test-inter --preset "http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=inter&menuAccent=subtle&menuColor=default&radius=default" --template next -c "C:/Yeasin/experiment/test-preset"
```

### Verifying the /init Endpoint Directly

You can test the init endpoint directly with curl:

```bash
# Should return valid JSON (after fix)
curl "http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist-sans&menuAccent=subtle&menuColor=default&radius=default&template=next"

# Should return error for invalid font
curl "http://localhost:4000/init?base=radix&style=vega&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist&menuAccent=subtle&menuColor=default&radius=default"
# Expected: {"error":"Invalid enum value. Expected 'geist-sans' | 'inter' | ..."}
```

### Valid Font Values

The following font values are accepted by the `/init` endpoint:
